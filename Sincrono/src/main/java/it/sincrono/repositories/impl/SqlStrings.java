...
public interface SqlStrings {
	...
	// SQL ANAGRAFICA
	public final String SQL_LIST_ANAGRAFICA_FILTER = "SELECT DISTINCT a.id, a.nome, a.cognome, e.nominativo, a.mail_aziendale, a.cellulare_privato, a.cf, c.data_fine_rapporto,e.data_fine FROM anagrafica a INNER JOIN storico_contratti b ON a.id = b.id_anagrafica INNER JOIN contratto c ON b.id_contratto = c.id INNER JOIN storico_commesse d ON a.id = d.id_anagrafica INNER JOIN commessa e ON d.id_commessa = e.id INNER JOIN tipo_livelli_contrattuali f ON f.id=c.id_tipo_livello INNER JOIN tipo_contratto g ON g.id=c.id_tipo_contratto INNER JOIN tipo_contratto_nazionale h ON h.id=c.id_contratto_nazionale INNER JOIN tipo_azienda i ON i.id=c.id_tipo_azienda WHERE 1=1 {0}";
