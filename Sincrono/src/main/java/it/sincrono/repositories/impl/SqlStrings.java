...
public interface SqlStrings {
	...
	// SQL ANAGRAFICA
	public final String SQL_LIST_ANAGRAFICA = "SELECT a.*,c.*,f.*,g.*,h.*,i.*,e.id FROM anagrafica a INNER JOIN storico_contratti b ON a.id = b.id_anagrafica INNER JOIN contratto c ON b.id_contratto = c.id INNER JOIN storico_commesse d ON a.id = d.id_anagrafica INNER JOIN commessa e ON d.id_commessa = e.id INNER JOIN tipo_livelli_contrattuali f ON f.id=c.id_tipo_livello INNER JOIN tipo_contratto g ON g.id=c.id_tipo_contratto INNER JOIN tipo_ccnl h ON h.id=c.id_tipo_ccnl INNER JOIN tipo_azienda i ON i.id=c.id_tipo_azienda WHERE a.attivo = true AND e.stato = true AND c.id=(select max(c1.id) from contratto c1 inner join storico_contratti s on s.id_contratto=c1.id where s.id_anagrafica=a.id)";
