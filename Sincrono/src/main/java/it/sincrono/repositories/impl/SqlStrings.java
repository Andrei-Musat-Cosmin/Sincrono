...
public interface SqlStrings {
	...
	public final String SQL_LIST_COMMESSE = "select distinct a.id,a.nome,a.cognome,a.codice_fiscale,d.* from commessa d inner join storico_commesse sc on d.id=sc.id_commessa inner join anagrafica a on sc.id_anagrafica=a.id inner join storico_contratti scc on sc.id_anagrafica=a.id inner join contratto c on c.id=scc.id_contratto where  d.attivo=true AND {0}";
	public final String SQL_LIST_ALL_COMMESSE = "select distinct a.id,a.nome,a.cognome,a.codice_fiscale,d.* from commessa d inner join storico_commesse sc on d.id=sc.id_commessa inner join anagrafica a on sc.id_anagrafica=a.id inner join storico_contratti scc on sc.id_anagrafica=a.id inner join contratto c on c.id=scc.id_contratto where  d.attivo=true ";
