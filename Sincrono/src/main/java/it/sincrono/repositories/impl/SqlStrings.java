...
public interface SqlStrings {
	...
	public final String SQL_DETTAGLIO_ANAGRAFICA_DTO = "SELECT a.*,c.*,d.*,e.*,f.*,g.*,h.*,i.*,n.* FROM anagrafica a INNER JOIN storico_contratti b ON a.id = b.id_anagrafica INNER JOIN contratto c ON b.id_contratto = c.id INNER JOIN tipo_livelli_contrattuali d ON d.id=c.id_tipo_livello INNER JOIN tipo_contratto e ON e.id=c.id_tipo_contratto INNER JOIN tipo_ccnl f ON f.id=c.id_tipo_ccnl INNER JOIN tipo_azienda g ON g.id=c.id_tipo_azienda INNER JOIN tipo_canale_reclutamento h ON h.id=c.id_tipo_canale_reclutamento LEFT OUTER JOIN tipo_causa_fine_rapporto i ON i.id=c.id_tipo_causa_fine_rapporto INNER JOIN utenti l ON a.id_utente=l.id INNER JOIN profili m ON m.id_utente =l.id INNER JOIN ruoli n ON m.id_ruolo=n.id WHERE a.id = 1 AND c.id=(select max(a1.id) from contratto a1 	inner join storico_contratti b on b.id_contratto=a1.id 	where b.id_anagrafica=a.id)";
	...
	public final String SQL_STORICO_CONTRATTI = "select c.*,tc.descrizione,tl.livello,tl.ccnl,tl.minimi_ret_23,ta.descrizione,tcn.descrizione from contratto c  inner join storico_contratti s  on c.id=s.id_contratto  inner join tipo_contratto tc  on c.id_tipo_contratto=tc.id  inner join tipo_livelli_contrattuali tl  on c.id_tipo_livello=tl.id  inner join tipo_azienda ta  on c.id_tipo_azienda=ta.id  inner join tipo_ccnl tcn  on c.id_tipo_ccnl=tcn.id  where s.id_anagrafica={0} and c.id>0";
	...
	public final String SQL_GET_ANAGRAFICA_BY_TOKEN = "SELECT a.*, d.*, e.*, b.id FROM anagrafica a INNER JOIN utenti b ON b.id=a.id_utente INNER JOIN profili c ON c.id_utente=b.id INNER JOIN ruoli d on c.id_ruolo=d.id INNER JOIN tipo_azienda e ON e.id = a.id_tipo_azienda WHERE b.token_password LIKE '{0}'";
