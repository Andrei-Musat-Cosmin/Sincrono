...
public interface SqlStrings {
	...
	public final String SQL_CURRENT_COMESSA="select * from commessa c where c.id=(select  c.id from storico_commesse st inner join anagrafica a on a.id=st.id_anagrafica inner join commessa c on c.id=st.id_commessa where c.stato=1 and a.id={0}";
